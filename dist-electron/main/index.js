  ...
  switch (connection.endpointType) {
    ...
    case "TabbyAPI":
      ...
      try {
        ...
        cancelTokenSource = axios.CancelToken.source();
        response = await axios.post(
          `${endpointURLObject.protocol}//${endpointURLObject.hostname}${endpointURLObject.port ? `:${endpointURLObject.port}` : ""}/v1/completions`,
          tabbyPayload,
          { cancelToken: cancelTokenSource.token, headers: {
            "Content-Type": "application/json",
            "x-api-key": password
...
const __filename$2 = url$1.fileURLToPath(typeof document === "undefined" ? require("url").pathToFileURL(__filename).href : document.currentScript && document.currentScript.src || new URL("index.js", document.baseURI).href);
const __dirname$2 = path$1.dirname(__filename$2);
process.env.DIST_ELECTRON = path$1.join(__dirname$2, "../");
...
const __filename$1 = url$1.fileURLToPath(typeof document === "undefined" ? require("url").pathToFileURL(__filename).href : document.currentScript && document.currentScript.src || new URL("index.js", document.baseURI).href);
const __dirname$1 = path$1.dirname(__filename$1);
process.env.DIST_ELECTRON = path$1.join(__dirname$1, "../");
process.env.DIST = path$1.join(process.env.DIST_ELECTRON, "../dist");
process.env.VITE_PUBLIC = process.env.VITE_DEV_SERVER_URL ? path$1.join(process.env.DIST_ELECTRON, "../public") : process.env.DIST;
